...
public class EntityCommand implements CommandExecutor {
    ...
    private boolean ActionAdd(Player player, String[] args) {
        ...
        eargs = null;
        if (ret != 1) {
            player.sendMessage(ChatColor.DARK_GREEN + Language.translate("commands.entity.regist.error"));
            SoundUtils.PlaySound(player,"cancel5", false);
            return false;
        }
                ...
                try {
                    ...
                    Bukkit.getScheduler().runTask(KuroBase.GetCitizens(), new Runnable() {
                        ...
                        public void run() {
                            ...
                            eargs = null;
                            if (ret != 1) {
                                player.sendMessage(ChatColor.DARK_GREEN + Language.translate("commands.entity.skin.fail"));
                                SoundUtils.PlaySound(player,"cancel5", false);
                                return;
                            }
    ...
    private boolean ActionDel(Player player, String[] args) {
        ...
        eargs = null;
        if (ret != 1) {
            player.sendMessage(ChatColor.DARK_GREEN + Language.translate("commands.entity.delete.error"));
            SoundUtils.PlaySound(player,"cancel5", false);
            return false;
        }
    ...
    private boolean ActionJoin(Player player, String[] args) {
        ...
        eargs.add(new DatabaseArgs("c", player.getUniqueId().toString())); // player uuid
        eargs.add(new DatabaseArgs("c", entity)); // name
        int ret = plugin.getDB().ExecuteUpdate(Language.translate("SQL.UPDATE.UUID.ENTITY"), eargs);
        eargs.clear();
        eargs = null;
        if (ret != 1) {
            npc.destroy();
            player.sendMessage(ChatColor.DARK_GREEN + Language.translate("commands.entity.join.error"));
            SoundUtils.PlaySound(player,"cancel5", false);
            return false;
        }
        ...
        // save
        //KuroBase.GetCitizens().storeNPCs();
    ...
    private boolean ActionQuit(Player player, String[] args) {
        ...
        if (uuid_str.length() > 0) {
            ...
            npc.despawn();
            player.sendMessage(ChatColor.DARK_RED + Language.translate("commands.entity.select.despawn"));
            SoundUtils.PlaySound(player,"switch1", false);
